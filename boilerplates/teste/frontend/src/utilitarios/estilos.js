...
export function estilizarBotao(cor_tema = "green") {
  const cor_botao = cor_tema;
  return `p-button-sm h-2rem text-base w-auto md:w-min mr-2
bg-${cor_botao}-600 border-${cor_botao}-800 shadow-6`;
}

export function estilizarBotaoRemover(cor_tema = "bluegray") {
  const cor_borda = cor_tema;
...
export function estilizarDivBotoesAcao() {
...
export function estilizarBotaoTabela(cor_tema, consultado = false) {
  let cor_botao = `text-${cor_tema}-900`;
  if (consultado) cor_botao = "p-button-danger";
  return `w-1rem p-button-sm p-button-text w-1 h-1rem ${cor_botao} shadow-6`;
}
